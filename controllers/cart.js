...
exports.getCartProductById = async function (req, res) {
    try {
        ...
        if (!cartProduct) return res.status(404).json({ message: "Product not found" });
...
exports.addToCart = async function (req, res) {
    ...
    try {
        ...
        const existingCartItem = userCartProduct.find((item) =>
        ...
        if (!cartProduct) {
            await session.abortTransaction();
            return res.status(500).json({ message: "Product not added to cart" });
        }
        ...
        const updatedProduct = await Product.findByIdAndUpdate(
            { _id: productId, countInStock: { $gt: cartProduct.quantity } },
...
exports.removeFromCart = async function (req, res) {
    ...
    try {
        ...
        if (!user.cart.includes(req.params.cartProductId)) {
            session.abortTransaction();
            return res.status(400).json({ message: "Product not in User's cart" });
        }
